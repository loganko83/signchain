...
## 접속 정보
- **프론트엔드**: https://trendy.storydot.kr/signchain/
- **API 엔드포인트**: https://trendy.storydot.kr/signchain/api/
- **데이터베이스**: PostgreSQL (signchain@localhost:5432)
- **프로세스 관리**: PM2 (ID: 5, 이름: signchain)

## 서버 구성 요약
```
Internet → Apache (443) → ProxyPass /signchain → Node.js (5001) → PostgreSQL (5432)
                 ↓
